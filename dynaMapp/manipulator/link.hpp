...
/**
 * @brief Link a genral class template for manipulator link 
 * model 
 * @details A n degree of freedom (dof) serial manipulator 
 * is composed of n links. This class describe the property
 * of a link. A n dof robot has n instance of the class Link.
 * 
 * @param joint_type = 1 for revolute and 0 for prismatric 
 * @note we use in this ifrst implmentation a simple lugre friction model
...
public:
    ...
    //! @brief  class full constructor 
    Link(const Int     joint_type, 
        const Real     q,
        const Real     d, 
        const Real     a, 
        const Real     alpha, 
    ...
    Matrix3d get_inertia(){return this->Jm;}
    ...
    //! @brief set the link center of mass coorduinates vector 
    ...
    //! @return the link joint overhaull stiffness  
...
private:
    ...
    Vector3d r,                 ///< Position of center of mass respect link coordinate system (min_para=F).
   ...
   Real c0,c1,c2;               ///< friction coefficents in the DC-Brushedd motor, consntes  from datasheet
   Real sensor_stiffness,       ///< joint torque sensor stiffness, consta value from datasheet
