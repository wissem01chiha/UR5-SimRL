...
template <typename Real, typename Int>
void  Link<Real, Int>::set_joint_position(Real q_){
    if (this->joint_type == 0){
...
template <typename Real, typename Int>
void Link<Real, Int>::update_time(){this->t += constant::TIME_STEP<Real>; }
...
void Link<Real, Int>::update_qm(Real current, Real voltage){   
    ...
    this->qm += this->qpm * constant::TIME_STEP<Real>;
}
...
template <typename Real, typename Int>
Real Link<Real, Int>::contact_friction_torque() noexcept {   
