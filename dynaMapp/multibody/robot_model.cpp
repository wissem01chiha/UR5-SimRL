...
{ 
    ...
    for (Int i = 0; i < ndof; i++){
        ...
        this->links[i].set_joint_poistion(q[i]);
...
template <typename Real, typename Int, size_t ndof>
Real Robot<Real, Int, ndof>::get_joint_position(const Int &i){
    return this->links[i].get_joint_position(i);
}

template <typename Real, typename Int, size_t ndof>
Real Robot<Real, Int, ndof>::get_joint_velocity(const Int &i){
    return this->links[i].get_joint_velocity(i);
}

template <typename Real, typename Int, size_t ndof>
Real Robot<Real, Int, ndof>::get_joint_acceleration(const Int &i){
    return this->links[i].get_joint_acceleration(i);
}

template <typename Real, typename Int, size_t ndof>
VectorXd Robot<Real, Int, ndof>::get_joint_position() const {
    VectorXd result = VectorXd::Zero(ndof);
    for (size_t i = 0; i < ndof; i++){
        result[i]=this->links[i].get_joint_position();
    }
    return result;
}

template <typename Real, typename Int, size_t ndof>
VectorXd Robot<Real, Int, ndof>::get_joint_velocity(){
    VectorXd result = VectorXd::Zero(ndof);
    for (size_t i = 0; i < ndof; i++){
        result[i] = this->links[i].get_joint_velocity();
    }
    return result;
}

template <typename Real, typename Int, size_t ndof>
VectorXd Robot<Real, Int, ndof>::get_joint_acceleration(){
    VectorXd result = VectorXd::Zero(ndof);
    for (size_t i = 0; i < ndof; i++){
        result[i]=this->links[i].get_joint_acceleration();
    }
    return result;
}

template <typename Real, typename Int, size_t ndof>
void  Robot<Real, Int, ndof>::set_joint_poistion(const Int &i, const Real &qi_){
    this->links[i].set_joint_poistion(qi_); }

template <typename Real, typename Int, size_t ndof>
void  Robot<Real, Int, ndof>::set_joint_velocity(const Int &i, const Real &qpi_){
    this->links[i].set_joint_velocity(qpi_); }

template <typename Real, typename Int, size_t ndof>
void Robot<Real, Int, ndof>::set_joint_acceleration(const Int &i, const Real &qppi_){
    this->links[i].set_joint_acceleration(qppi_); }

template <typename Real, typename Int, size_t ndof>
void  Robot<Real, Int, ndof>::set_joint_poistion(const Real (&q_)[ndof]){
    for (Int i = 0; i < ndof; i++){
        set_joint_poistion(i,q_[i]);
    } 
}

template <typename Real, typename Int, size_t ndof>
void Robot<Real, Int, ndof>::set_joint_velocity(const Real (&qp_)[ndof]){
    for (Int i = 0; i < ndof; i++){
        set_joint_velocity(i, qp_[i]);
    }
}

template <typename Real, typename Int, size_t ndof>
void  Robot<Real, Int, ndof>::set_joint_acceleration(const Real (&qpp_)[ndof]){
    for (Int i = 0; i < ndof; i++){
        set_joint_acceleration(i, qpp_[i]);
    }
}

template <typename Real, typename Int, size_t ndof>
void Robot<Real, Int, ndof>::set_joint_status(const Int i){
     if (!this->links[i].is_static()){
        this->links[i].set_joint_status(true);
     }
}
                                        ...
                                        const Real (&Next_)[ndof])
{
   ...
   Matrix3d Rt, temp;
   set_joint_poistion(q_);
   set_joint_velocity(qp_);
   set_joint_acceleration(qpp_);
   ...
   for (Int i = 0; i < ndof; i++)
   {
        ...
        if (this->links[i].get_joint_type()==0)
        {
            ...
            this->wp[i] = Rt*(wp[i-1] + z0*qpp(i)
                    + this->w[i-1].cross(this->z0 * get_joint_velocity(i)));
...
VectorXd Robot<Real, Int, ndof>::torque_novelocity(const Real (&qpp_)[ndof])
{
    ...
    for (Int i = 0; i < ndof; i++){
        ...
        if (this->links[i].joint_type == 0){
            this->wp[i] = Rt *(this->wp[i-1] + this->z0*get_joint_acceleration(i));
            this->vp[i] = this->wp[i].cross(this->p[i])+ Rt * this->vp[i];
        }else{
            this->wp[i] = Rt * this->wp[i-1];
            this->vp[i] = Rt * (this->vp[i-1] + this->z0*get_joint_acceleration(i))
                 + wp[i].cross(this->p[i]);
        }
    ...
    for (Int i = ndof-1; i => 0; i--){
        ...
        ltorque(i) = temp(1,1)+
        this->links[i].Jm*this->links[i].get_ratio()*links[i].get_ratio()*get_joint_acceleration(i);
    }
    return ltorque;
}
