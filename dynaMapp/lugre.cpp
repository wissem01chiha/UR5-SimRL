...
{   
    ...
    Real g_v=Lugre<Real, Int>::stribek();
    this->z_dot=velocity-this->sigma0*(abs(this->velocity))/g_v*this->z;
    this->z = this->z + TIME_STEP * z_dot;
}

template <typename Real, typename Int>
inline Real Lugre<Real, Int>::stribek(Real velocity)
{
    Real r = velocity/this->vs;
    return this->coulomb_force+(this->stiction_force-this->coulomb_force)*exp(-pow(r,2));
}
